...
namespace ConsoleApp6
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bool userAutorized;// Авторизация пользователя
            {
                userAutorized = false;
                ...
                bool authorizationAttempAvalible = authorizationAttempCounter < ALLOWABLE_NUMBER_OF_AUTHORIZATION_ATTEMPTS;

                while(authorizationAttempAvalible)
                {
                    string login, password; // ввод логина и пароля
                    {
                    ...
                    // попытка авторизации пользователя
                    {
                        ...
                        while (index < loginList.Length && index < passwordList.Length) // можно сделать одну проверку
                        {
                            bool loginMatched, passwordMatched; // сопастовление логина и пароля
                            {
                    ...
                    else
                    {
                        ...
                        authorizationAttempAvalible = ++authorizationAttempCounter < ALLOWABLE_NUMBER_OF_AUTHORIZATION_ATTEMPTS;

                        if (authorizationAttempAvalible)
                        {
                            continue;
                        }
                        else
